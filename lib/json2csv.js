    ...
    if (dataElement && Object.getOwnPropertyNames(dataElement).length > 0) {
      ...
      params.fields.forEach(function (fieldElement) {
        ...
        if (val !== undefined) {
          ...
          // console.log(stringifiedElement);

          if (typeof val === 'object') stringifiedElement = JSON.stringify(stringifiedElement);

          if (params.quotes !== '"') {
            console.log(fieldElement, val, typeof params.quotes)
            var quotes = typeof params.quotes === 'function' ? params.quotes(fieldElement, val) : params.quotes;
            stringifiedElement = replaceQuotationMarks(stringifiedElement, quotes);
          }
