...
function checkParams(params, callback) {
  ...
  params.quotes = typeof params.quotes === 'undefined' ? '"' : params.quotes;

  //#check double quotes
  params.doubleQuotes = typeof params.doubleQuotes === 'undefined' ? Array(3).join(params.quotes) : params.doubleQuotes;
 ...
 */
function replaceQuotationMarks(stringifiedElement, fieldElement, quotes) {
  if (typeof quotes === 'undefined') {
    quotes = fieldElement;
    fieldElement = undefined;
  }

  var quoteVal = typeof quotes === 'function' ? quotes(stringifiedElement, fieldElement) : quotes;
  ...
  if (stringifiedElement[0] === '"' && stringifiedElement[lastCharIndex] === '"') {
    ...
    splitElement[0] = quoteVal;
    splitElement[lastCharIndex] = quoteVal;
    ...
    if (dataElement && Object.getOwnPropertyNames(dataElement).length > 0) {
      ...
      params.fields.forEach(function (fieldElement) {
        ...
        if (val !== undefined) {
          ...
          if (params.quotes !== '"') {
            stringifiedElement = replaceQuotationMarks(stringifiedElement, fieldElement, params.quotes);
          }
